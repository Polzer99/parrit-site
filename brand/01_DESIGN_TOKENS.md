...
| Rôle | Famille | Usage |
|---|---|---|
| Display / titres | `Geist` | titres, sous-titres, héros |
| Corps / labels / boutons | `Geist Mono` | tout le descriptif, les petits labels, le texte des boutons |

- Titres Geist : `letter-spacing: -0.04em` systématique, poids `500`–`600`, héros en `clamp(40px, 6vw, 72px)`.
...
Chargement :
- Site Next : `next/font` (déjà en place dans les layouts).
- Artifacts, PDF, decks, propales : embed base64 depuis `node_modules/next/dist/next-devtools/server/font/geist-latin.woff2` et `geist-mono-latin.woff2`.
...
Frames Figma de référence (vérité pixel) : `design-source/figma-template/Parrit Template 1..6.svg`.
Banque photo : `design-source/figma-template/photos/`. Traitement photo : `mix-blend-mode: soft-light`, angles nets.
