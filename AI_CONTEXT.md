...
L'application publique canonique vit dans `src/app/(rev01)/` : `/`, `/manufacture`, `/standard`, `/dossiers`, `/commission`, `/journal`, `/journal/[slug]`, `/journal/rss.xml`, `/legal` et `/sketch/[id]`. Le layout du groupe charge les polices, tokens, styles et la command bar. La home simplifiée porte `Opening`, `QuickCapture` hero et `AgentEsquisse`, puis les métriques et les sections éditoriales. Les cinq pages intérieures ont leur copy FR/EN dédié.
